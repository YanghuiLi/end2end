...
def getOrderedDict(samples, prefix='intent-', delimiter=';'):
    ...
    rodList = OrderedDict([(val, key) for (key, val) in odList.items()])
...
class DataSetCSV(object):
    ...
    def _load_data(self):
        ...
        with open(self.csv_file, 'r') as fcsv:
    ...
    def _get_params(self):
        ...
        for (tag, tag_id) in self.userTag2id.items():  # <pad> is included
        ...
        self.id2userTagIntent = OrderedDict(
            [(val, key) for (key, val) in self.userIntent2id.items()])
        ...
        self.id2userTagIntentAgentPrevAct = OrderedDict(
            [(val, key) for (key, val) in self.userTagIntentAgentPrevAct2id.items()])
